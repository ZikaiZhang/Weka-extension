...
public class WeightedMajorityAlgorithm02 extends AbstractClassifier {
	...
	private int numTestInstance = 0;
	
	private double[][] predArray;
	
	// method for setting the number of test instances
	public void setNumTestInstance(int instanceNum) {
		this.numTestInstance = instanceNum;
	}
	
	public void setPredArray() {
		predArray = new double[3][numTestInstance];
	}
	
	public double[][] getPredArray() {
		
		double[][] newPredArray = new double[3][numTestInstance];
		
		for (int i = 0; i < 3; i++) {
			newPredArray[i] = predArray[i].clone();
		}
	...
	{	
		for (int index = 0; index < 3; index++) {
			predArray[index][i] = ensemble[index].classifyInstance(instance);
		}
